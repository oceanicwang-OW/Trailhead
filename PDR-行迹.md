...
| # | 任务 | 产出 / 验收 | 依赖 |
|---|---|---|---|
| T6.1 | 时间线内拖动重排 POI，写回 order | 重排后重启仍保持 | T5.1, T1.4 |
| T6.2 | 删除 / 替换某 POI（替换时从候选集再选） | 操作后路线与交通段自动重算 | T6.1, T3.5 |
...
| 6 | T6.* 编辑/重排 | ⬜ 待办 | 视图已可选中,落编辑写回待补 |
| 7 | T7.* 设置 UI | ✅ 完成(UI) | API/用量/缓存 UI;用量真实统计待接入 |
...
T6 编辑/重排(拖动写回 order、删除/替换重算路线)、T8.2 配额降级横幅、T7.2 真实用量统计。顺序仍按 §8 编号推进。
