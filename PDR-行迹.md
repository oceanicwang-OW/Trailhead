...
| T6.2 | 删除 / 替换某 POI（替换时从候选集再选） | 🟡 删除并重算已接；替换候选待接 | T6.1, T3.5 |
...
| 6 | T6.* 编辑/重排 | 🟡 部分 | T6.1 POI 重排已写回;T6.2a 删除并重算已接;T6.2b 替换与 T6.3 单日重生成待补 |
...
T6.2b 替换候选并重算路线、T6.3 单日重新生成、T7.2 真实用量统计、T8.2 配额降级横幅。顺序仍按 §8 编号推进。
