...
public enum DayRouter {
    ...
    public static func route(_ stops: [POICandidate],
                             ...
                             exitAnchor: (lat: Double, lng: Double)? = nil,
                             maxIterations: Int = 20) -> [POICandidate] {
        routeWithDiagnostics(stops, entryAnchor: entryAnchor, exitAnchor: exitAnchor,
                             maxIterations: maxIterations).tour
    }
    ...
    public static func routeWithDiagnostics(_ stops: [POICandidate],
        ...
        let exit = exitAnchor.map {
            POICandidate(id: "__exit__", name: "", kind: .sight, subtype: "", lat: $0.lat, lng: $0.lng)
        }
        ...
        return twoOpt(tour, entryAnchor: anchor, exitAnchor: exit, maxIterations: maxIterations)
    }
    ...
    private static func twoOpt(_ initial: [POICandidate], entryAnchor: POICandidate?,
                               exitAnchor: POICandidate?,
        ...
        while improved, iter < maxIterations {
            ...
            for i in 0..<(tour.count - 1) {
                for j in (i + 1)..<tour.count {
                    ...
                    if pathLength(candidate, entryAnchor: entryAnchor, exitAnchor: exitAnchor) + 1e-6
                        < pathLength(tour, entryAnchor: entryAnchor, exitAnchor: exitAnchor) {
                        tour = candidate
                        improved = true
                    }
                }
    ...
    private static func pathLength(_ tour: [POICandidate], entryAnchor: POICandidate?,
                                   exitAnchor: POICandidate?) -> Double {
        var total = 0.0
        if let entryAnchor, let first = tour.first { total += d(entryAnchor, first) }
        for i in 0..<max(0, tour.count - 1) { total += d(tour[i], tour[i + 1]) }
        if let exitAnchor, let last = tour.last { total += d(last, exitAnchor) }
        return total
    }
